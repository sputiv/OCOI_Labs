...
import numpy as np
import pandas as pd
from .noise_generator import RandomNoise, NoiseModel
...
class AccelerometerData:
    ...
    def load_from_csv(self, file_path: str) -> None:
        """Загрузка данных акселерометра из CSV файла"""
        ...
        for i, axis in enumerate(['x', 'y', 'z']):
            ...
            if use_noised_data and axis in self.noised_channels:
                axes[i].plot(self.seconds_elapsed, self.noised_channels[axis], 
                            label='С помехой', linewidth=1, alpha=0.4)
            ...
            axes[i].set_ylabel(f'Ускорение {axis} (м/с²)')
            axes[i].legend()
            axes[i].grid(True, alpha=0.3)
        
        axes[2].set_xlabel('Время (сек)')
        plt.suptitle('Данные акселерометра', fontsize=14)
        plt.tight_layout()
        plt.show()
    ...
    def model_info(self):
        ...
        print("\n" + "="*50)
        print("ИНФОРМАЦИЯ О ШУМАХ АКСЕЛЕРОМЕТРА:")
        print("="*50)
...
if __name__ == "__main__":
    # Создание экземпляра данных с загрузкой из CSV
    acc_data = AccelerometerData('data/Accelerometer.csv')
    
    # Создание моделей помех для каждой оси
    x_noise = RandomNoise(0.4)
    y_noise = RandomNoise(0.1)
    z_noise = RandomNoise(0.3)
    
    # Наложение помех на каждую ось отдельно
    noisy_x = acc_data.apply_noise('x', x_noise)
    noisy_y = acc_data.apply_noise('y', y_noise)
